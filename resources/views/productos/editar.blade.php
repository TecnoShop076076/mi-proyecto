...
<body>
    ...
    <div class="form-box">
        <form action="{{ route('productos.actualizar', $producto->id_producto) }}" method="POST" enctype="multipart/form-data">
            @csrf

            <label>Nombre *</label>
            <input type="text" name="nombre" value="{{ old('nombre', $producto->nombre) }}" required maxlength="150">
            @error('nombre')<div class="error">{{ $message }}</div>@enderror

            <label>Código *</label>
            <input type="text" name="codigo" value="{{ old('codigo', $producto->codigo) }}" required maxlength="50">
            ...
            <select name="proveedor">
                ...
                @foreach($proveedores as $prov)
                    <option value="{{ $prov->id_proveedor }}" {{ old('proveedor', $producto->proveedor) == $prov->id_proveedor ? 'selected' : '' }}>
                        {{ $prov->razonsocial }}
                    </option>
                @endforeach
            </select>
